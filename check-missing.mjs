...
// The correct full IMDb Top 250 IDs
const top250 = [
  ...
  'tt0174480','tt0477348','tt0119372','tt0993846','tt1454468',
  'tt0092099','tt0363163','tt0405159','tt1392214','tt0477347',
  'tt0338564','tt1375670','tt0268380','tt0457492','tt0947798',
  'tt1798709','tt0816692','tt0816692','tt0816692','tt0816692',
...
async function run() {
  ...
  const existingMap = new Map(existing.map(m => [m.imdb_id, m.title]))

  console.log(`IDs in top250 list: ${uniqueIds.length}`)
  console.log(`Movies in your DB: ${existing.length}`)
  console.log(`\nMissing from DB:`)

  const missing = []
  for (const id of uniqueIds) {
    if (!existingMap.has(id)) {
      // Look up the title from OMDB
      const res = await fetch(`https://www.omdbapi.com/?i=${id}&apikey=${OMDB_API_KEY}`)
      const data = await res.json()
      const title = data.Response === 'True' ? data.Title : 'Unknown'
